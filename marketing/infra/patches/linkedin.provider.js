...
let LinkedinProvider = class LinkedinProvider extends social_abstract_1.SocialAbstract {
    ...
    async checkValidity(posts, vals) {
    ...
    async waitForMediaToBeReady(urn, accessToken, type, maxAttempts = 20, intervalMs = 30000) {
        const label = type === 'videos' ? 'video' : type === 'documents' ? 'document' : 'image';
        for (let attempt = 0; attempt < maxAttempts; attempt += 1) {
            const json = await (await this.fetch(`https://api.linkedin.com/rest/${type}/${encodeURIComponent(urn)}`, {
                method: 'GET',
                headers: {
                    'X-Restli-Protocol-Version': '2.0.0',
                    'LinkedIn-Version': '202601',
            ...
            if (status === 'PROCESSING_FAILED') {
                throw new social_abstract_1.BadBody(this.identifier, JSON.stringify(json), '{}', `LinkedIn ${label} processing failed${processingFailureReason ? `: ${processingFailureReason}` : ''}`);
            }
            await (0, timer_1.timer)(intervalMs);
        }
        throw new social_abstract_1.BadBody(this.identifier, '{}', '{}', `Timed out waiting for LinkedIn ${label} to be ready`);
    }
    fixText(text) {
    ...
    async createMainPost(id, accessToken, firstPost, mediaIds, type, isPdf) {
        ...
        if (response.status !== 201 && response.status !== 200) {
            throw new social_abstract_1.BadBody(this.identifier, '{}', JSON.stringify(postPayload), 'Error posting to LinkedIn');
        }
    ...
    async createCommentPost(id, accessToken, post, parentPostId, type) {
        ...
        const response = await this.fetch(`https://api.linkedin.com/rest/socialActions/${encodeURIComponent(parentPostId)}/comments`, {
            method: 'POST',
            headers: {
